...
bool test_FunctionCoroutine_fibonacci_2__next(void* _ctx) {
    ...
    for (ctx->i = 1; ctx->i < ctx->n; ctx->i++) {
        ...
        ctx->b = ctx->tmp;
        #line 42 "src/compiler/FunctionCoroutine.pv"
...
struct trait_Iter_u64VTable test_FunctionCoroutine_fibonacci_2__VTABLE__ITER = { .fn_next = test_FunctionCoroutine_fibonacci_2__next, .fn_value = test_FunctionCoroutine_fibonacci_2__value };
