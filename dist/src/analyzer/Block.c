...
bool Block__parse_expression_statement(struct Block* self, struct Context* context, struct Generics* generics, struct StatementData* data) {
    ...
    if (Context__check_value(context, TOKEN_TYPE__SYMBOL, "=") || Context__check_value(context, TOKEN_TYPE__SYMBOL, "+=") || Context__check_value(context, TOKEN_TYPE__SYMBOL, "-=") || Context__check_value(context, TOKEN_TYPE__SYMBOL, "*=") || Context__check_value(context, TOKEN_TYPE__SYMBOL, "/=") || Context__check_value(context, TOKEN_TYPE__SYMBOL, "|=")) {
        ...
        if (!is_pointer_arithmetic) {
            ...
            if (Token__eq(operator, TOKEN_TYPE__SYMBOL, "=")) {
                #line 45 "src/analyzer/Block.pv"
                AssignmentStatement__validate_and_update_scope(context, expression, value);
            } else {
...
struct Scope* Block__parse_preprocessor_branch(struct Block* self, struct Context* context, struct Generics* generics) {
    ...
    while (!Context__check_value(context, TOKEN_TYPE__KEYWORD, "#elif") && !Context__check_value(context, TOKEN_TYPE__KEYWORD, "#else") && !Context__check_value(context, TOKEN_TYPE__KEYWORD, "#endif")) {
        #line 163 "src/analyzer/Block.pv"
        if (Context__current(context) == 0) {
            #line 164 "src/analyzer/Block.pv"
            Context__error(context, "Expected #endif");
        ...
        if (!Block__parse_statement(self, context, generics)) {
    ...
    struct Scope* result = ArenaAllocator__store_Scope(context->allocator, scope);
