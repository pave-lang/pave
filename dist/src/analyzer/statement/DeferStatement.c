...
struct DeferStatement* DeferStatement__parse(struct Block* parent_block, struct Context* context, struct Generics* generics) {
    ...
    if (Context__check_value(context, TOKEN_TYPE__SYMBOL, "{")) {
        ...
        return ArenaAllocator__store_DeferStatement(context->allocator, (struct DeferStatement[]){(struct DeferStatement) { .type = DEFER_STATEMENT__BLOCK, .block_value = defer_block }});
    } else {
        ...
        return ArenaAllocator__store_DeferStatement(context->allocator, (struct DeferStatement[]){(struct DeferStatement) { .type = DEFER_STATEMENT__EXPRESSION, .expression_value = expression }});
    }
}
