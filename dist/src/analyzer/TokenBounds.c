...
void TokenBounds__include(struct TokenBounds* self, struct TokenBounds bounds) {
    ...
    if (bounds.first->start_line < self->first->start_line || (bounds.first->start_line == self->first->start_line && bounds.first->start_column < self->first->start_column)) {
        #line 50 "src/analyzer/Token.pv"
        self->first = bounds.first;
    }

    #line 53 "src/analyzer/Token.pv"
    if (bounds.last->end_line > self->last->end_line || (bounds.last->end_line == self->last->end_line && bounds.last->end_column > self->last->end_column)) {
