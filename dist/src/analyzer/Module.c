...
#include <tuple_str_ref_ConstVariable.h>
...
struct Module* Module__new(struct Namespace* namespace, struct str path) {
    ...
    *self = (struct Module) {
        ...
        .const_variables = HashMap_str_ref_ConstVariable__new((struct trait_Allocator) { .vtable = &ARENA_ALLOCATOR__VTABLE__ALLOCATOR, .instance = allocator }),
        .const_types = HashMap_str_Type__new((struct trait_Allocator) { .vtable = &ARENA_ALLOCATOR__VTABLE__ALLOCATOR, .instance = allocator }),
    };
...
bool Module__parse(struct Module* self) {
    ...
    while (context->pos < context->length && result) {
        ...
        if (Token__eq(token, TOKEN_TYPE__KEYWORD, "ns")) {
        ...
        } else if (Token__eq(token, TOKEN_TYPE__KEYWORD, "const")) {
            #line 192 "src/analyzer/Module.pv"
            if (!Context__expect_value(context, TOKEN_TYPE__KEYWORD, "const")) {
                #line 192 "src/analyzer/Module.pv"
                return false;
            }
            ...
            struct ConstVariable* const_var = ArenaAllocator__Allocator__alloc(allocator, sizeof(struct ConstVariable));
            #line 205 "src/analyzer/Module.pv"
            *const_var = (struct ConstVariable) {
                .name = const_name,
                .module = self,
            ...
            HashMap_str_ref_ConstVariable__insert(&self->const_variables, const_name->value, const_var);
...
bool Module__fill_namespace(struct Module* self) {
    ...
    { struct HashMapIter_str_ref_ConstVariable __iter = HashMap_str_ref_ConstVariable__iter(&self->const_variables);
    #line 291 "src/analyzer/Module.pv"
    while (HashMapIter_str_ref_ConstVariable__next(&__iter)) {
        #line 291 "src/analyzer/Module.pv"
        struct str name = HashMapIter_str_ref_ConstVariable__value(&__iter)->_0;
        #line 291 "src/analyzer/Module.pv"
        struct ConstVariable* const_var = HashMapIter_str_ref_ConstVariable__value(&__iter)->_1;

        #line 292 "src/analyzer/Module.pv"
        HashMap_str_Type__insert(&self->const_types, name, (struct Type) { .type = TYPE__CONST, .const_value = const_var });
    } }
...
bool Module__parse_consts(struct Module* self) {
    ...
    while (HashMapIter_str_ref_ConstVariable__next(&__iter)) {
        ...
        ConstVariable__parse_types(const_var);
    } }
...
struct Type* Module__find_value(struct Module* self, struct str name) {
    ...
    struct Type* type = HashMap_str_Type__find(&self->const_types, &name);
    ...
    while (Iter_ref_ref_Namespace__next(&__iter)) {
        ...
        while (HashMapIter_str_ref_Module__next(&__iter)) {
            ...
            type = HashMap_str_Type__find(&module->const_types, &name);
