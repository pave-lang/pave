...
struct HashMap_str_ref_Trait HashMap_str_ref_Trait__new(struct Allocator allocator) {
    #line 36 "src/std/HashMap.pv"
    int32_t initial_capacity = 16;
    #line 37 "src/std/HashMap.pv"
    struct HashMap_str_ref_Trait self = (struct HashMap_str_ref_Trait) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_ref_Trait__resize(&self, initial_capacity);
...
void HashMap_str_ref_Trait__resize(struct HashMap_str_ref_Trait* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_Trait*));
    #line 48 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_Trait));
...
struct Trait** HashMap_str_ref_Trait__insert(struct HashMap_str_ref_Trait* self, struct str key, struct Trait* value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Trait) { .key = key, .value = value };
...
struct HashMap_str_ref_Trait HashMap_str_ref_Trait__clone(struct HashMap_str_ref_Trait* self, struct Allocator allocator) {
    ...
    struct HashMap_str_ref_Trait other = (struct HashMap_str_ref_Trait) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_Trait)),
        .length = self->length,
