...
struct HashMap_str_Struct HashMap_str_Struct__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_Struct__with_capacity(allocator, 16);
}
...
struct HashMap_str_Struct HashMap_str_Struct__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_Struct self = (struct HashMap_str_Struct) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_Struct__resize(&self, capacity);
...
void HashMap_str_Struct__resize(struct HashMap_str_Struct* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Struct*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Struct));
    #line 56 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 57 "src/std/HashMap.pv"
    HashMap_str_Struct__fill_buckets(self);
}
...
struct Struct* HashMap_str_Struct__find(struct HashMap_str_Struct* self, struct str* key) {
    ...
    struct HashMapBucket_str_Struct* current_bucket_node = self->buckets[bucket_index];
...
struct Struct* HashMap_str_Struct__insert(struct HashMap_str_Struct* self, struct str key, struct Struct value) {
    ...
    struct Struct* existing_value = HashMap_str_Struct__find(self, &key);
    ...
    if (self->capacity == 0 || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_Struct__resize(self, self->capacity * 2);
    }
    ...
    self->data[self->length] = (struct HashMapBucket_str_Struct) { .key = key, .value = value, .next = 0 };
    #line 97 "src/std/HashMap.pv"
    struct HashMapBucket_str_Struct* data = self->data + self->length;
...
bool HashMap_str_Struct__remove(struct HashMap_str_Struct* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (str__Hash__hash(&current_bucket_node->key) == hash) {
            ...
            struct HashMapBucket_str_Struct* last = self->data + self->length - 1;
...
void HashMap_str_Struct__fill_buckets(struct HashMap_str_Struct* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_Struct*));

    #line 141 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 142 "src/std/HashMap.pv"
        struct HashMapBucket_str_Struct* node = self->data + i;
...
struct HashMap_str_Struct HashMap_str_Struct__clone(struct HashMap_str_Struct* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Struct other = (struct HashMap_str_Struct) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Struct)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 164 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_Struct));
    #line 165 "src/std/HashMap.pv"
    HashMap_str_Struct__fill_buckets(&other);
...
struct HashMapIter_str_Struct HashMap_str_Struct__iter(struct HashMap_str_Struct* self) {
    ...
    return (struct HashMapIter_str_Struct) {
        .iter = self->data - 1,
...
void HashMap_str_Struct__clear(struct HashMap_str_Struct* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_Struct));
...
struct HashMapBucket_str_Struct* HashMap_str_Struct__Index__index(void* __self) {
    struct HashMap_str_Struct* self = __self; (void)self;
