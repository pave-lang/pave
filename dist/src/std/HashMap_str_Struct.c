...
struct HashMap_str_Struct HashMap_str_Struct__new(struct Allocator allocator) {
    ...
    struct HashMap_str_Struct self = (struct HashMap_str_Struct) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_Struct__resize(&self, initial_capacity);
...
struct Struct* HashMap_str_Struct__insert(struct HashMap_str_Struct* self, struct str key, struct Struct value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_Struct) { .key = key, .value = value };
