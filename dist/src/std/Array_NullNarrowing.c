...
struct Array_NullNarrowing Array_NullNarrowing__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_NullNarrowing) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_NullNarrowing Array_NullNarrowing__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_NullNarrowing self = (struct Array_NullNarrowing) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_NullNarrowing__reserve(&self, length);
...
void Array_NullNarrowing__reserve(struct Array_NullNarrowing* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct NullNarrowing));
...
uintptr_t Array_NullNarrowing__append(struct Array_NullNarrowing* self, struct NullNarrowing value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_NullNarrowing__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_NullNarrowing__prepend(struct Array_NullNarrowing* self, struct NullNarrowing value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct NullNarrowing));
...
bool Array_NullNarrowing__remove_back(struct Array_NullNarrowing* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct NullNarrowing));
...
void Array_NullNarrowing__clear(struct Array_NullNarrowing* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct NullNarrowing));
...
struct Array_NullNarrowing Array_NullNarrowing__clone(struct Array_NullNarrowing* self, struct trait_Allocator allocator) {
    ...
    struct NullNarrowing* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct NullNarrowing));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct NullNarrowing));

    #line 176 "src/std/Array.pv"
    return (struct Array_NullNarrowing) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct slice_NullNarrowing Array_NullNarrowing__as_slice(struct Array_NullNarrowing* self) {
    ...
    return (struct slice_NullNarrowing) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct NullNarrowing* Array_NullNarrowing__Index__index(void* __self) {
    struct Array_NullNarrowing* self = __self; (void)self;
