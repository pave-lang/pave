...
struct Array_CXCursor Array_CXCursor__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_CXCursor) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_CXCursor Array_CXCursor__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_CXCursor self = (struct Array_CXCursor) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_CXCursor__reserve(&self, length);
...
uintptr_t Array_CXCursor__append(struct Array_CXCursor* self, CXCursor value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_CXCursor__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_CXCursor__prepend(struct Array_CXCursor* self, CXCursor value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(CXCursor));
...
bool Array_CXCursor__remove_back(struct Array_CXCursor* self) {
    ...
    memset(self->data + self->length, 0, sizeof(CXCursor));
...
void Array_CXCursor__clear(struct Array_CXCursor* self) {
    ...
    memset(self->data, 0, self->length * sizeof(CXCursor));
