...
struct HashMap_str_ref_Global HashMap_str_ref_Global__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_ref_Global__with_capacity(allocator, 16);
}
...
struct HashMap_str_ref_Global HashMap_str_ref_Global__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_ref_Global self = (struct HashMap_str_ref_Global) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_ref_Global__resize(&self, capacity);
...
void HashMap_str_ref_Global__resize(struct HashMap_str_ref_Global* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_Global*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_Global));
    #line 56 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 57 "src/std/HashMap.pv"
    HashMap_str_ref_Global__fill_buckets(self);
}
...
struct Global** HashMap_str_ref_Global__find(struct HashMap_str_ref_Global* self, struct str* key) {
    ...
    struct HashMapBucket_str_ref_Global* current_bucket_node = self->buckets[bucket_index];
...
struct Global** HashMap_str_ref_Global__insert(struct HashMap_str_ref_Global* self, struct str key, struct Global* value) {
    ...
    struct Global** existing_value = HashMap_str_ref_Global__find(self, &key);
    ...
    if (self->capacity == 0 || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_ref_Global__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_ref_Global* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Global) { .key = key, .value = value, .next = 0 };
    #line 99 "src/std/HashMap.pv"
    struct HashMapBucket_str_ref_Global* data = self->data + self->length;
...
bool HashMap_str_ref_Global__remove(struct HashMap_str_ref_Global* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (str__Hash__hash(&current_bucket_node->key) == hash) {
            ...
            struct HashMapBucket_str_ref_Global* last = self->data + self->length - 1;
...
void HashMap_str_ref_Global__fill_buckets(struct HashMap_str_ref_Global* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Global*));

    #line 143 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 144 "src/std/HashMap.pv"
        struct HashMapBucket_str_ref_Global* node = self->data + i;
...
struct HashMap_str_ref_Global HashMap_str_ref_Global__clone(struct HashMap_str_ref_Global* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Global other = (struct HashMap_str_ref_Global) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_Global)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 169 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_ref_Global));
    #line 170 "src/std/HashMap.pv"
    HashMap_str_ref_Global__fill_buckets(&other);
...
struct HashMapIter_str_ref_Global HashMap_str_ref_Global__iter(struct HashMap_str_ref_Global* self) {
    ...
    return (struct HashMapIter_str_ref_Global) {
        .iter = self->data - 1,
...
void HashMap_str_ref_Global__clear(struct HashMap_str_ref_Global* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Global));
...
struct HashMapBucket_str_ref_Global* HashMap_str_ref_Global__Index__index(void* __self) {
    struct HashMap_str_ref_Global* self = __self; (void)self;
...
struct trait_Map_str_ref_GlobalVTable HASH_MAP_STR_REF_GLOBAL__VTABLE__MAP = { .fn_find = &HashMap_str_ref_Global__Map_str_ref_Global__find, .fn_insert = &HashMap_str_ref_Global__Map_str_ref_Global__insert, .fn_remove = &HashMap_str_ref_Global__Map_str_ref_Global__remove };
