...
struct HashMap_str_ref_Global HashMap_str_ref_Global__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Global self = (struct HashMap_str_ref_Global) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_ref_Global__resize(&self, initial_capacity);
...
struct Global** HashMap_str_ref_Global__insert(struct HashMap_str_ref_Global* self, struct str key, struct Global* value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Global) { .key = key, .value = value };
