...
void HashMap_str_ref_Global__resize(struct HashMap_str_ref_Global* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_Global*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_Global));
...
struct HashMap_str_ref_Global HashMap_str_ref_Global__clone(struct HashMap_str_ref_Global* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Global other = (struct HashMap_str_ref_Global) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_Global)),
        .length = self->length,
