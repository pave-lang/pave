...
struct Array_SymbolInfo Array_SymbolInfo__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_SymbolInfo) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_SymbolInfo Array_SymbolInfo__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_SymbolInfo self = (struct Array_SymbolInfo) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_SymbolInfo__reserve(&self, length);
...
void Array_SymbolInfo__reserve(struct Array_SymbolInfo* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct SymbolInfo));
...
uintptr_t Array_SymbolInfo__append(struct Array_SymbolInfo* self, struct SymbolInfo value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_SymbolInfo__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_SymbolInfo__prepend(struct Array_SymbolInfo* self, struct SymbolInfo value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct SymbolInfo));
...
bool Array_SymbolInfo__remove_back(struct Array_SymbolInfo* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct SymbolInfo));
...
void Array_SymbolInfo__clear(struct Array_SymbolInfo* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct SymbolInfo));
...
struct Array_SymbolInfo Array_SymbolInfo__clone(struct Array_SymbolInfo* self, struct trait_Allocator allocator) {
    ...
    struct SymbolInfo* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct SymbolInfo));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct SymbolInfo));

    #line 176 "src/std/Array.pv"
    return (struct Array_SymbolInfo) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct slice_SymbolInfo Array_SymbolInfo__as_slice(struct Array_SymbolInfo* self) {
    ...
    return (struct slice_SymbolInfo) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct SymbolInfo* Array_SymbolInfo__Index__index(void* __self) {
    struct Array_SymbolInfo* self = __self; (void)self;
