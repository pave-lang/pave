...
struct Array_TypeImpl Array_TypeImpl__new(struct Allocator allocator) {
    #line 78 "src/std/Array.pv"
    return (struct Array_TypeImpl) { .allocator = allocator };
}

#line 81 "src/std/Array.pv"
struct Array_TypeImpl Array_TypeImpl__new_with_length(struct Allocator allocator, uintptr_t length) {
...
struct Array_TypeImpl Array_TypeImpl__new_with_capacity(struct Allocator allocator, uintptr_t length) {
...
void Array_TypeImpl__reserve(struct Array_TypeImpl* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, capacity * sizeof(struct TypeImpl));
...
uintptr_t Array_TypeImpl__append(struct Array_TypeImpl* self, struct TypeImpl value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_TypeImpl__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_TypeImpl__prepend(struct Array_TypeImpl* self, struct TypeImpl value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct TypeImpl));
...
bool Array_TypeImpl__remove_back(struct Array_TypeImpl* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct TypeImpl));
...
void Array_TypeImpl__clear(struct Array_TypeImpl* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct TypeImpl));
...
struct Array_TypeImpl Array_TypeImpl__clone(struct Array_TypeImpl* self, struct Allocator allocator) {
    #line 173 "src/std/Array.pv"
    struct TypeImpl* data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct TypeImpl));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct TypeImpl));

    #line 176 "src/std/Array.pv"
    return (struct Array_TypeImpl) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct ArrayIter_ref_TypeImpl Array_TypeImpl__iter(struct Array_TypeImpl* self) {
    ...
    return ArrayIter_ref_TypeImpl__new((void*)(self->data), (void*)(self->data + self->length));
}

#line 189 "src/std/Array.pv"
struct slice_TypeImpl Array_TypeImpl__as_slice(struct Array_TypeImpl* self) {
    #line 190 "src/std/Array.pv"
    return (struct slice_TypeImpl) { .data = self->data + 0, .length = self->length - 0 };
}
