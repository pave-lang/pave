...
void HashMap_u32_Type__resize(struct HashMap_u32_Type* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_u32_Type*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_u32_Type));
...
struct Type* HashMap_u32_Type__find(struct HashMap_u32_Type* self, uint32_t* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(u32__Hash__hash(&current_bucket_node->key), hash) && u32__Eq_u32__eq(current_bucket_node->key, *key)) {
...
struct Type* HashMap_u32_Type__insert(struct HashMap_u32_Type* self, uint32_t key, struct Type value) {
    ...
    struct HashMapBucket_u32_Type* bucket_node = *current_bucket_node;
...
void HashMap_u32_Type__fill_buckets(struct HashMap_u32_Type* self) {
    ...
    for (uintptr_t i = 0; i < self->length; i++) {
        ...
        Hash hash = u32__Hash__hash(&(*node).key);
...
struct HashMap_u32_Type HashMap_u32_Type__clone(struct HashMap_u32_Type* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_u32_Type other = (struct HashMap_u32_Type) {
        ...
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_u32_Type)),
        .length = self->length,
...
struct HashMapBucket_u32_Type* HashMap_u32_Type__Index__index(void* __self) {
    struct HashMap_u32_Type* self = __self; (void)self;
...
struct trait_Map_u32_TypeVTable HASH_MAP_U32_TYPE__VTABLE__MAP = { .fn_find = &HashMap_u32_Type__Map_u32_Type__find, .fn_insert = &HashMap_u32_Type__Map_u32_Type__insert, .fn_remove = &HashMap_u32_Type__Map_u32_Type__remove };
