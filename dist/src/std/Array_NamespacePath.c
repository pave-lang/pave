...
struct Array_NamespacePath Array_NamespacePath__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_NamespacePath) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_NamespacePath Array_NamespacePath__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_NamespacePath self = (struct Array_NamespacePath) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
...
void Array_NamespacePath__reserve(struct Array_NamespacePath* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct NamespacePath));
...
struct Array_NamespacePath Array_NamespacePath__clone(struct Array_NamespacePath* self, struct trait_Allocator allocator) {
    ...
    struct NamespacePath* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct NamespacePath));
...
struct slice_NamespacePath Array_NamespacePath__as_slice(struct Array_NamespacePath* self) {
    ...
    return (struct slice_NamespacePath) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct NamespacePath* Array_NamespacePath__Index__index(void* __self) {
    struct Array_NamespacePath* self = __self; (void)self;
