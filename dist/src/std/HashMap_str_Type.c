...
struct HashMap_str_Type HashMap_str_Type__new(struct Allocator allocator) {
    #line 36 "src/std/HashMap.pv"
    int32_t initial_capacity = 16;
    #line 37 "src/std/HashMap.pv"
    struct HashMap_str_Type self = (struct HashMap_str_Type) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_Type__resize(&self, initial_capacity);
...
void HashMap_str_Type__resize(struct HashMap_str_Type* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Type*));
    #line 48 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Type));
...
struct Type* HashMap_str_Type__insert(struct HashMap_str_Type* self, struct str key, struct Type value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_Type) { .key = key, .value = value };
...
struct HashMap_str_Type HashMap_str_Type__clone(struct HashMap_str_Type* self, struct Allocator allocator) {
    ...
    struct HashMap_str_Type other = (struct HashMap_str_Type) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Type)),
        .length = self->length,
