...
struct Array_char Array_char__new(struct Allocator allocator) {
    #line 78 "src/std/Array.pv"
    return (struct Array_char) { .allocator = allocator };
}

#line 81 "src/std/Array.pv"
struct Array_char Array_char__new_with_length(struct Allocator allocator, uintptr_t length) {
    #line 82 "src/std/Array.pv"
    struct Array_char self = (struct Array_char) { .allocator = allocator };
    #line 83 "src/std/Array.pv"
    Array_char__reserve(&self, length);
...
struct Array_char Array_char__new_with_capacity(struct Allocator allocator, uintptr_t length) {
...
uintptr_t Array_char__append(struct Array_char* self, char value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_char__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_char__prepend(struct Array_char* self, char value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(char));
...
bool Array_char__remove_back(struct Array_char* self) {
    ...
    memset(self->data + self->length, 0, sizeof(char));
...
struct Array_char Array_char__clone(struct Array_char* self, struct Allocator allocator) {
...
struct slice_char Array_char__as_slice(struct Array_char* self) {
    ...
    return (struct slice_char) { .data = self->data + 0, .length = self->length - 0 };
}
