...
struct Array_TypeId Array_TypeId__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_TypeId) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_TypeId Array_TypeId__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_TypeId self = (struct Array_TypeId) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
...
void Array_TypeId__reserve(struct Array_TypeId* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(TypeId));
...
struct Array_TypeId Array_TypeId__clone(struct Array_TypeId* self, struct trait_Allocator allocator) {
    ...
    TypeId* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(TypeId));
...
TypeId* Array_TypeId__Index__index(void* __self) {
    struct Array_TypeId* self = __self; (void)self;
