...
struct Array_ref_Scope Array_ref_Scope__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_ref_Scope) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_ref_Scope Array_ref_Scope__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_ref_Scope self = (struct Array_ref_Scope) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_ref_Scope__reserve(&self, length);
...
void Array_ref_Scope__reserve(struct Array_ref_Scope* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct Scope*));
...
uintptr_t Array_ref_Scope__append(struct Array_ref_Scope* self, struct Scope* value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_ref_Scope__reserve(self, self->capacity + additional_capacity);
    }
...
struct Array_ref_Scope Array_ref_Scope__clone(struct Array_ref_Scope* self, struct trait_Allocator allocator) {
    ...
    struct Scope** data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct Scope*));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Scope*));

    #line 176 "src/std/Array.pv"
    return (struct Array_ref_Scope) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct slice_ref_Scope Array_ref_Scope__as_slice(struct Array_ref_Scope* self) {
    ...
    return (struct slice_ref_Scope) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct Scope** Array_ref_Scope__Index__index(void* __self) {
    struct Array_ref_Scope* self = __self; (void)self;
