...
void HashMap_str_ref_ImplConst__resize(struct HashMap_str_ref_ImplConst* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_ImplConst*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_ImplConst));
...
struct HashMap_str_ref_ImplConst HashMap_str_ref_ImplConst__clone(struct HashMap_str_ref_ImplConst* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_ImplConst other = (struct HashMap_str_ref_ImplConst) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_ImplConst)),
        .length = self->length,
