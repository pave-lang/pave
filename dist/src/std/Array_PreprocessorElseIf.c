...
struct Array_PreprocessorElseIf Array_PreprocessorElseIf__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_PreprocessorElseIf) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_PreprocessorElseIf Array_PreprocessorElseIf__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_PreprocessorElseIf self = (struct Array_PreprocessorElseIf) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_PreprocessorElseIf__reserve(&self, length);
...
void Array_PreprocessorElseIf__reserve(struct Array_PreprocessorElseIf* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct PreprocessorElseIf));
...
uintptr_t Array_PreprocessorElseIf__append(struct Array_PreprocessorElseIf* self, struct PreprocessorElseIf value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_PreprocessorElseIf__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_PreprocessorElseIf__prepend(struct Array_PreprocessorElseIf* self, struct PreprocessorElseIf value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct PreprocessorElseIf));
...
bool Array_PreprocessorElseIf__remove_back(struct Array_PreprocessorElseIf* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct PreprocessorElseIf));
...
void Array_PreprocessorElseIf__clear(struct Array_PreprocessorElseIf* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct PreprocessorElseIf));
...
struct Array_PreprocessorElseIf Array_PreprocessorElseIf__clone(struct Array_PreprocessorElseIf* self, struct trait_Allocator allocator) {
    ...
    struct PreprocessorElseIf* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct PreprocessorElseIf));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct PreprocessorElseIf));

    #line 176 "src/std/Array.pv"
    return (struct Array_PreprocessorElseIf) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct slice_PreprocessorElseIf Array_PreprocessorElseIf__as_slice(struct Array_PreprocessorElseIf* self) {
    ...
    return (struct slice_PreprocessorElseIf) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct PreprocessorElseIf* Array_PreprocessorElseIf__Index__index(void* __self) {
    struct Array_PreprocessorElseIf* self = __self; (void)self;
