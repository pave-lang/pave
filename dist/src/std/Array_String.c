...
struct Array_String Array_String__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_String) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_String Array_String__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_String self = (struct Array_String) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_String__reserve(&self, length);
...
void Array_String__reserve(struct Array_String* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct String));
...
uintptr_t Array_String__append(struct Array_String* self, struct String value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_String__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_String__prepend(struct Array_String* self, struct String value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct String));
...
bool Array_String__remove_back(struct Array_String* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct String));
...
void Array_String__clear(struct Array_String* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct String));
...
struct Array_String Array_String__clone(struct Array_String* self, struct trait_Allocator allocator) {
    ...
    struct String* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct String));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct String));

    #line 176 "src/std/Array.pv"
    return (struct Array_String) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct String* Array_String__Index__index(void* __self) {
    struct Array_String* self = __self; (void)self;
