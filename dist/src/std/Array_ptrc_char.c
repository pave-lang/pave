...
void Array_ptrc_char__reserve(struct Array_ptrc_char* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(char const*));
...
uintptr_t Array_ptrc_char__append(struct Array_ptrc_char* self, char const* value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_ptrc_char__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_ptrc_char__prepend(struct Array_ptrc_char* self, char const* value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(char const*));
...
bool Array_ptrc_char__remove_back(struct Array_ptrc_char* self) {
    ...
    memset(self->data + self->length, 0, sizeof(char const*));
...
void Array_ptrc_char__clear(struct Array_ptrc_char* self) {
    ...
    memset(self->data, 0, self->length * sizeof(char const*));
...
struct Array_ptrc_char Array_ptrc_char__clone(struct Array_ptrc_char* self, struct trait_Allocator allocator) {
    ...
    char const** data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(char const*));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(char const*));

    #line 176 "src/std/Array.pv"
    return (struct Array_ptrc_char) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
char const** Array_ptrc_char__Index__index(void* __self) {
    struct Array_ptrc_char* self = __self; (void)self;
