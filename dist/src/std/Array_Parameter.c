...
struct Array_Parameter Array_Parameter__new(struct Allocator allocator) {
    ...
    return (struct Array_Parameter) { .allocator = allocator };
}
...
struct Array_Parameter Array_Parameter__new_with_length(struct Allocator allocator, uintptr_t length) {
    ...
    struct Array_Parameter self = (struct Array_Parameter) { .allocator = allocator };
    #line 83 "src/std/Array.pv"
    Array_Parameter__reserve(&self, length);
...
void Array_Parameter__reserve(struct Array_Parameter* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, capacity * sizeof(struct Parameter));
...
uintptr_t Array_Parameter__append(struct Array_Parameter* self, struct Parameter value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_Parameter__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_Parameter__prepend(struct Array_Parameter* self, struct Parameter value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct Parameter));
...
bool Array_Parameter__remove_back(struct Array_Parameter* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct Parameter));
...
void Array_Parameter__clear(struct Array_Parameter* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct Parameter));
...
struct Array_Parameter Array_Parameter__clone(struct Array_Parameter* self, struct Allocator allocator) {
    ...
    struct Parameter* data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct Parameter));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Parameter));

    #line 176 "src/std/Array.pv"
    return (struct Array_Parameter) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct ArrayIter_ref_Parameter Array_Parameter__iter(struct Array_Parameter* self) {
    ...
    return ArrayIter_ref_Parameter__new((void*)(self->data), (void*)(self->data + self->length));
}
