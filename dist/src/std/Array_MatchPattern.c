...
struct Array_MatchPattern Array_MatchPattern__new(struct Allocator allocator) {
    ...
    return (struct Array_MatchPattern) { .allocator = allocator };
}
...
struct Array_MatchPattern Array_MatchPattern__new_with_length(struct Allocator allocator, uintptr_t length) {
    ...
    struct Array_MatchPattern self = (struct Array_MatchPattern) { .allocator = allocator };
    #line 83 "src/std/Array.pv"
    Array_MatchPattern__reserve(&self, length);
...
void Array_MatchPattern__reserve(struct Array_MatchPattern* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, capacity * sizeof(struct MatchPattern));
...
uintptr_t Array_MatchPattern__append(struct Array_MatchPattern* self, struct MatchPattern value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_MatchPattern__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_MatchPattern__prepend(struct Array_MatchPattern* self, struct MatchPattern value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct MatchPattern));
...
bool Array_MatchPattern__remove_back(struct Array_MatchPattern* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct MatchPattern));
...
void Array_MatchPattern__clear(struct Array_MatchPattern* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct MatchPattern));
...
struct Array_MatchPattern Array_MatchPattern__clone(struct Array_MatchPattern* self, struct Allocator allocator) {
    ...
    struct MatchPattern* data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct MatchPattern));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct MatchPattern));

    #line 176 "src/std/Array.pv"
    return (struct Array_MatchPattern) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct ArrayIter_ref_MatchPattern Array_MatchPattern__iter(struct Array_MatchPattern* self) {
    ...
    return ArrayIter_ref_MatchPattern__new((void*)(self->data), (void*)(self->data + self->length));
}
