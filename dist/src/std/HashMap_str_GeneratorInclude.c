...
struct HashMap_str_GeneratorInclude HashMap_str_GeneratorInclude__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_GeneratorInclude self = (struct HashMap_str_GeneratorInclude) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_GeneratorInclude__resize(&self, initial_capacity);
...
void HashMap_str_GeneratorInclude__resize(struct HashMap_str_GeneratorInclude* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_GeneratorInclude*));
    #line 48 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_GeneratorInclude));
    #line 49 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 50 "src/std/HashMap.pv"
    HashMap_str_GeneratorInclude__fill_buckets(self);
}
...
struct GeneratorInclude* HashMap_str_GeneratorInclude__find(struct HashMap_str_GeneratorInclude* self, struct str* key) {
    ...
    struct HashMapBucket_str_GeneratorInclude* current_bucket_node = self->buckets[bucket_index];
...
struct GeneratorInclude* HashMap_str_GeneratorInclude__insert(struct HashMap_str_GeneratorInclude* self, struct str key, struct GeneratorInclude value) {
    ...
    struct GeneratorInclude* existing_value = HashMap_str_GeneratorInclude__find(self, &key);
    ...
    if (self->capacity == 0 || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_GeneratorInclude__resize(self, self->capacity * 2);
    }
    ...
    self->data[self->length] = (struct HashMapBucket_str_GeneratorInclude) { .key = key, .value = value };
    #line 90 "src/std/HashMap.pv"
    struct HashMapBucket_str_GeneratorInclude* data = self->data + self->length;
...
bool HashMap_str_GeneratorInclude__remove(struct HashMap_str_GeneratorInclude* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (str__Hash__hash(&current_bucket_node->key) == hash) {
            ...
            struct HashMapBucket_str_GeneratorInclude* last = self->data + self->length - 1;
...
void HashMap_str_GeneratorInclude__fill_buckets(struct HashMap_str_GeneratorInclude* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_GeneratorInclude*));

    #line 134 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 135 "src/std/HashMap.pv"
        struct HashMapBucket_str_GeneratorInclude* node = self->data + i;
...
struct HashMap_str_GeneratorInclude HashMap_str_GeneratorInclude__clone(struct HashMap_str_GeneratorInclude* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_GeneratorInclude other = (struct HashMap_str_GeneratorInclude) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_GeneratorInclude)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 157 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_GeneratorInclude));
    #line 158 "src/std/HashMap.pv"
    HashMap_str_GeneratorInclude__fill_buckets(&other);
...
struct HashMapIter_str_GeneratorInclude HashMap_str_GeneratorInclude__iter(struct HashMap_str_GeneratorInclude* self) {
    ...
    return (struct HashMapIter_str_GeneratorInclude) {
        .iter = self->data - 1,
...
void HashMap_str_GeneratorInclude__clear(struct HashMap_str_GeneratorInclude* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_GeneratorInclude));
