...
struct Array_ImplNode Array_ImplNode__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_ImplNode) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_ImplNode Array_ImplNode__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_ImplNode self = (struct Array_ImplNode) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_ImplNode__reserve(&self, length);
...
void Array_ImplNode__reserve(struct Array_ImplNode* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct ImplNode));
...
uintptr_t Array_ImplNode__append(struct Array_ImplNode* self, struct ImplNode value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_ImplNode__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_ImplNode__prepend(struct Array_ImplNode* self, struct ImplNode value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct ImplNode));
...
bool Array_ImplNode__remove_back(struct Array_ImplNode* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct ImplNode));
...
void Array_ImplNode__clear(struct Array_ImplNode* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct ImplNode));
...
struct Array_ImplNode Array_ImplNode__clone(struct Array_ImplNode* self, struct trait_Allocator allocator) {
    ...
    struct ImplNode* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct ImplNode));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct ImplNode));

    #line 176 "src/std/Array.pv"
    return (struct Array_ImplNode) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct ImplNode* Array_ImplNode__Index__index(void* __self) {
    struct Array_ImplNode* self = __self; (void)self;
