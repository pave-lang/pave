...
#include <std/HashMapBucket_str_i64.h>
#include <usize.h>
#include <std/Hash.h>
...
struct HashMap_str_i64 HashMap_str_i64__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_i64__with_capacity(allocator, 16);
}
...
struct HashMap_str_i64 HashMap_str_i64__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_i64 self = (struct HashMap_str_i64) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_i64__resize(&self, capacity);
...
void HashMap_str_i64__resize(struct HashMap_str_i64* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_i64*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_i64));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_i64__fill_buckets(self);
}
...
int64_t* HashMap_str_i64__find(struct HashMap_str_i64* self, struct str* key) {
    ...
    struct HashMapBucket_str_i64* current_bucket_node = self->buckets[bucket_index];
...
int64_t* HashMap_str_i64__insert(struct HashMap_str_i64* self, struct str key, int64_t value) {
    ...
    int64_t* existing_value = HashMap_str_i64__find(self, &key);
    ...
    if (usize__Eq_usize__eq(self->capacity, 0) || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_i64__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_i64* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_i64) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_i64* data = self->data + self->length;
...
bool HashMap_str_i64__remove(struct HashMap_str_i64* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_i64* last = self->data + self->length - 1;
...
void HashMap_str_i64__fill_buckets(struct HashMap_str_i64* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_i64*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_i64* node = self->data + i;
...
struct HashMap_str_i64 HashMap_str_i64__clone(struct HashMap_str_i64* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_i64 other = (struct HashMap_str_i64) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_i64)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_i64));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_i64__fill_buckets(&other);
...
struct HashMapIter_str_i64 HashMap_str_i64__iter(struct HashMap_str_i64* self) {
    ...
    return (struct HashMapIter_str_i64) {
        .iter = self->data - 1,
...
void HashMap_str_i64__clear(struct HashMap_str_i64* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_i64));
...
struct HashMapBucket_str_i64* HashMap_str_i64__Index__index(void* __self) {
    struct HashMap_str_i64* self = __self; (void)self;
...
struct trait_Map_str_i64VTable HASH_MAP_STR_I64__VTABLE__MAP = { .fn_find = &HashMap_str_i64__Map_str_i64__find, .fn_insert = &HashMap_str_i64__Map_str_i64__insert, .fn_remove = &HashMap_str_i64__Map_str_i64__remove };
