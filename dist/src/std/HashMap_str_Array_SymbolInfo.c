...
struct HashMap_str_Array_SymbolInfo HashMap_str_Array_SymbolInfo__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_Array_SymbolInfo__with_capacity(allocator, 16);
}
...
struct HashMap_str_Array_SymbolInfo HashMap_str_Array_SymbolInfo__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_Array_SymbolInfo self = (struct HashMap_str_Array_SymbolInfo) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_Array_SymbolInfo__resize(&self, capacity);
...
void HashMap_str_Array_SymbolInfo__resize(struct HashMap_str_Array_SymbolInfo* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Array_SymbolInfo*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Array_SymbolInfo));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_Array_SymbolInfo__fill_buckets(self);
}
...
struct Array_SymbolInfo* HashMap_str_Array_SymbolInfo__find(struct HashMap_str_Array_SymbolInfo* self, struct str* key) {
    ...
    struct HashMapBucket_str_Array_SymbolInfo* current_bucket_node = self->buckets[bucket_index];
...
struct Array_SymbolInfo* HashMap_str_Array_SymbolInfo__insert(struct HashMap_str_Array_SymbolInfo* self, struct str key, struct Array_SymbolInfo value) {
    ...
    struct Array_SymbolInfo* existing_value = HashMap_str_Array_SymbolInfo__find(self, &key);
    ...
    if (usize__Eq_usize__eq(self->capacity, 0) || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_Array_SymbolInfo__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_Array_SymbolInfo* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_Array_SymbolInfo) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_Array_SymbolInfo* data = self->data + self->length;
...
bool HashMap_str_Array_SymbolInfo__remove(struct HashMap_str_Array_SymbolInfo* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_Array_SymbolInfo* last = self->data + self->length - 1;
...
void HashMap_str_Array_SymbolInfo__fill_buckets(struct HashMap_str_Array_SymbolInfo* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_Array_SymbolInfo*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i != self->length; i < self->length ? i++ : i--) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_Array_SymbolInfo* node = self->data + i;
...
struct HashMap_str_Array_SymbolInfo HashMap_str_Array_SymbolInfo__clone(struct HashMap_str_Array_SymbolInfo* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Array_SymbolInfo other = (struct HashMap_str_Array_SymbolInfo) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Array_SymbolInfo)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_Array_SymbolInfo));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_Array_SymbolInfo__fill_buckets(&other);
...
struct HashMapIter_str_Array_SymbolInfo HashMap_str_Array_SymbolInfo__iter(struct HashMap_str_Array_SymbolInfo* self) {
    ...
    return (struct HashMapIter_str_Array_SymbolInfo) {
        .iter = self->data - 1,
...
void HashMap_str_Array_SymbolInfo__clear(struct HashMap_str_Array_SymbolInfo* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_Array_SymbolInfo));
...
struct HashMapBucket_str_Array_SymbolInfo* HashMap_str_Array_SymbolInfo__Index__index(void* __self) {
    struct HashMap_str_Array_SymbolInfo* self = __self; (void)self;
...
struct trait_Map_str_Array_SymbolInfoVTable HASH_MAP_STR_ARRAY_SYMBOL_INFO__VTABLE__MAP = { .fn_find = &HashMap_str_Array_SymbolInfo__Map_str_Array_SymbolInfo__find, .fn_insert = &HashMap_str_Array_SymbolInfo__Map_str_Array_SymbolInfo__insert, .fn_remove = &HashMap_str_Array_SymbolInfo__Map_str_Array_SymbolInfo__remove };
