...
struct HashMap_str_bool HashMap_str_bool__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_bool__with_capacity(allocator, 16);
}
...
struct HashMap_str_bool HashMap_str_bool__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_bool self = (struct HashMap_str_bool) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_bool__resize(&self, capacity);
...
void HashMap_str_bool__resize(struct HashMap_str_bool* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_bool*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_bool));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_bool__fill_buckets(self);
}
...
bool* HashMap_str_bool__find(struct HashMap_str_bool* self, struct str* key) {
    ...
    struct HashMapBucket_str_bool* current_bucket_node = self->buckets[bucket_index];
...
bool* HashMap_str_bool__insert(struct HashMap_str_bool* self, struct str key, bool value) {
    ...
    bool* existing_value = HashMap_str_bool__find(self, &key);
    ...
    struct HashMapBucket_str_bool* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_bool) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_bool* data = self->data + self->length;
...
bool HashMap_str_bool__remove(struct HashMap_str_bool* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_bool* last = self->data + self->length - 1;
...
void HashMap_str_bool__fill_buckets(struct HashMap_str_bool* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_bool*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i != self->length; i < self->length ? i++ : i--) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_bool* node = self->data + i;
...
struct HashMap_str_bool HashMap_str_bool__clone(struct HashMap_str_bool* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_bool other = (struct HashMap_str_bool) {
        ...
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_bool)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_bool));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_bool__fill_buckets(&other);
...
struct HashMapIter_str_bool HashMap_str_bool__iter(struct HashMap_str_bool* self) {
    ...
    return (struct HashMapIter_str_bool) {
        .iter = self->data - 1,
...
void HashMap_str_bool__clear(struct HashMap_str_bool* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_bool));
...
struct HashMapBucket_str_bool* HashMap_str_bool__Index__index(void* __self) {
    struct HashMap_str_bool* self = __self; (void)self;
...
struct trait_Map_str_boolVTable HASH_MAP_STR_BOOL__VTABLE__MAP = { .fn_find = &HashMap_str_bool__Map_str_bool__find, .fn_insert = &HashMap_str_bool__Map_str_bool__insert, .fn_remove = &HashMap_str_bool__Map_str_bool__remove };
