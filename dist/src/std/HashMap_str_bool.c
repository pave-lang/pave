...
struct HashMap_str_bool HashMap_str_bool__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_bool self = (struct HashMap_str_bool) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_bool__resize(&self, initial_capacity);
...
bool* HashMap_str_bool__insert(struct HashMap_str_bool* self, struct str key, bool value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_bool) { .key = key, .value = value };
