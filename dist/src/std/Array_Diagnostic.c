...
struct Array_Diagnostic Array_Diagnostic__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_Diagnostic) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_Diagnostic Array_Diagnostic__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_Diagnostic self = (struct Array_Diagnostic) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_Diagnostic__reserve(&self, length);
...
void Array_Diagnostic__reserve(struct Array_Diagnostic* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct Diagnostic));
...
uintptr_t Array_Diagnostic__append(struct Array_Diagnostic* self, struct Diagnostic value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_Diagnostic__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_Diagnostic__prepend(struct Array_Diagnostic* self, struct Diagnostic value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct Diagnostic));
...
bool Array_Diagnostic__remove_back(struct Array_Diagnostic* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct Diagnostic));
...
void Array_Diagnostic__clear(struct Array_Diagnostic* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct Diagnostic));
...
struct Array_Diagnostic Array_Diagnostic__clone(struct Array_Diagnostic* self, struct trait_Allocator allocator) {
    ...
    struct Diagnostic* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct Diagnostic));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Diagnostic));

    #line 176 "src/std/Array.pv"
    return (struct Array_Diagnostic) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct Diagnostic* Array_Diagnostic__Index__index(void* __self) {
    struct Array_Diagnostic* self = __self; (void)self;
