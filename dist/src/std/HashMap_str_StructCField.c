...
struct HashMap_str_StructCField HashMap_str_StructCField__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_StructCField__with_capacity(allocator, 16);
}
...
struct HashMap_str_StructCField HashMap_str_StructCField__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_StructCField self = (struct HashMap_str_StructCField) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_StructCField__resize(&self, capacity);
...
void HashMap_str_StructCField__resize(struct HashMap_str_StructCField* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_StructCField*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_StructCField));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_StructCField__fill_buckets(self);
}
...
struct StructCField* HashMap_str_StructCField__find(struct HashMap_str_StructCField* self, struct str* key) {
    ...
    struct HashMapBucket_str_StructCField* current_bucket_node = self->buckets[bucket_index];
...
struct StructCField* HashMap_str_StructCField__insert(struct HashMap_str_StructCField* self, struct str key, struct StructCField value) {
    ...
    struct StructCField* existing_value = HashMap_str_StructCField__find(self, &key);
    ...
    if (usize__Eq_usize__eq(self->capacity, 0) || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_StructCField__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_StructCField* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_StructCField) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_StructCField* data = self->data + self->length;
...
bool HashMap_str_StructCField__remove(struct HashMap_str_StructCField* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_StructCField* last = self->data + self->length - 1;
...
void HashMap_str_StructCField__fill_buckets(struct HashMap_str_StructCField* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_StructCField*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i != self->length; i < self->length ? i++ : i--) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_StructCField* node = self->data + i;
...
struct HashMap_str_StructCField HashMap_str_StructCField__clone(struct HashMap_str_StructCField* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_StructCField other = (struct HashMap_str_StructCField) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_StructCField)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_StructCField));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_StructCField__fill_buckets(&other);
...
struct HashMapIter_str_StructCField HashMap_str_StructCField__iter(struct HashMap_str_StructCField* self) {
    ...
    return (struct HashMapIter_str_StructCField) {
        .iter = self->data - 1,
...
void HashMap_str_StructCField__clear(struct HashMap_str_StructCField* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_StructCField));
...
struct HashMapBucket_str_StructCField* HashMap_str_StructCField__Index__index(void* __self) {
    struct HashMap_str_StructCField* self = __self; (void)self;
...
struct trait_Map_str_StructCFieldVTable HASH_MAP_STR_STRUCT_CFIELD__VTABLE__MAP = { .fn_find = &HashMap_str_StructCField__Map_str_StructCField__find, .fn_insert = &HashMap_str_StructCField__Map_str_StructCField__insert, .fn_remove = &HashMap_str_StructCField__Map_str_StructCField__remove };
