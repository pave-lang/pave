...
struct HashMap_str_StructCField HashMap_str_StructCField__new(struct Allocator allocator) {
    ...
    struct HashMap_str_StructCField self = (struct HashMap_str_StructCField) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_StructCField__resize(&self, initial_capacity);
...
struct StructCField* HashMap_str_StructCField__insert(struct HashMap_str_StructCField* self, struct str key, struct StructCField value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_StructCField) { .key = key, .value = value };
