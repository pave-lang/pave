...
struct HashMap_str_ref_Namespace HashMap_str_ref_Namespace__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_ref_Namespace__with_capacity(allocator, 16);
}
...
struct HashMap_str_ref_Namespace HashMap_str_ref_Namespace__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_ref_Namespace self = (struct HashMap_str_ref_Namespace) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_ref_Namespace__resize(&self, capacity);
...
void HashMap_str_ref_Namespace__resize(struct HashMap_str_ref_Namespace* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_Namespace*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_Namespace));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_ref_Namespace__fill_buckets(self);
}
...
struct Namespace** HashMap_str_ref_Namespace__find(struct HashMap_str_ref_Namespace* self, struct str* key) {
    ...
    struct HashMapBucket_str_ref_Namespace* current_bucket_node = self->buckets[bucket_index];
...
struct Namespace** HashMap_str_ref_Namespace__insert(struct HashMap_str_ref_Namespace* self, struct str key, struct Namespace* value) {
    ...
    struct Namespace** existing_value = HashMap_str_ref_Namespace__find(self, &key);
    ...
    if (usize__Eq_usize__eq(self->capacity, 0) || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_ref_Namespace__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_ref_Namespace* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Namespace) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_ref_Namespace* data = self->data + self->length;
...
bool HashMap_str_ref_Namespace__remove(struct HashMap_str_ref_Namespace* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_ref_Namespace* last = self->data + self->length - 1;
...
void HashMap_str_ref_Namespace__fill_buckets(struct HashMap_str_ref_Namespace* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Namespace*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_ref_Namespace* node = self->data + i;
...
struct HashMap_str_ref_Namespace HashMap_str_ref_Namespace__clone(struct HashMap_str_ref_Namespace* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Namespace other = (struct HashMap_str_ref_Namespace) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_Namespace)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_ref_Namespace));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_ref_Namespace__fill_buckets(&other);
...
struct HashMapIter_str_ref_Namespace HashMap_str_ref_Namespace__iter(struct HashMap_str_ref_Namespace* self) {
    ...
    return (struct HashMapIter_str_ref_Namespace) {
        .iter = self->data - 1,
...
void HashMap_str_ref_Namespace__clear(struct HashMap_str_ref_Namespace* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Namespace));
...
struct HashMapBucket_str_ref_Namespace* HashMap_str_ref_Namespace__Index__index(void* __self) {
    struct HashMap_str_ref_Namespace* self = __self; (void)self;
...
struct trait_Map_str_ref_NamespaceVTable HASH_MAP_STR_REF_NAMESPACE__VTABLE__MAP = { .fn_find = &HashMap_str_ref_Namespace__Map_str_ref_Namespace__find, .fn_insert = &HashMap_str_ref_Namespace__Map_str_ref_Namespace__insert, .fn_remove = &HashMap_str_ref_Namespace__Map_str_ref_Namespace__remove };
