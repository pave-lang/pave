...
struct HashMap_usize_TypeFunctionUsage HashMap_usize_TypeFunctionUsage__new(struct Allocator allocator) {
    ...
    struct HashMap_usize_TypeFunctionUsage self = (struct HashMap_usize_TypeFunctionUsage) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_usize_TypeFunctionUsage__resize(&self, initial_capacity);
...
void HashMap_usize_TypeFunctionUsage__resize(struct HashMap_usize_TypeFunctionUsage* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_usize_TypeFunctionUsage*));
    #line 48 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_usize_TypeFunctionUsage));
...
struct TypeFunctionUsage* HashMap_usize_TypeFunctionUsage__insert(struct HashMap_usize_TypeFunctionUsage* self, uintptr_t key, struct TypeFunctionUsage value) {
    ...
    self->data[self->length] = (struct HashMapBucket_usize_TypeFunctionUsage) { .key = key, .value = value };
...
struct HashMap_usize_TypeFunctionUsage HashMap_usize_TypeFunctionUsage__clone(struct HashMap_usize_TypeFunctionUsage* self, struct Allocator allocator) {
    ...
    struct HashMap_usize_TypeFunctionUsage other = (struct HashMap_usize_TypeFunctionUsage) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_usize_TypeFunctionUsage)),
        .length = self->length,
