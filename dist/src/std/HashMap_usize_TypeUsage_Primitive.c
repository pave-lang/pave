...
struct HashMap_usize_TypeUsage_Primitive HashMap_usize_TypeUsage_Primitive__new(struct Allocator allocator) {
    ...
    struct HashMap_usize_TypeUsage_Primitive self = (struct HashMap_usize_TypeUsage_Primitive) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_usize_TypeUsage_Primitive__resize(&self, initial_capacity);
...
void HashMap_usize_TypeUsage_Primitive__resize(struct HashMap_usize_TypeUsage_Primitive* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_usize_TypeUsage_Primitive*));
    #line 48 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_usize_TypeUsage_Primitive));
...
struct TypeUsage_Primitive* HashMap_usize_TypeUsage_Primitive__insert(struct HashMap_usize_TypeUsage_Primitive* self, uintptr_t key, struct TypeUsage_Primitive value) {
    ...
    self->data[self->length] = (struct HashMapBucket_usize_TypeUsage_Primitive) { .key = key, .value = value };
...
struct HashMap_usize_TypeUsage_Primitive HashMap_usize_TypeUsage_Primitive__clone(struct HashMap_usize_TypeUsage_Primitive* self, struct Allocator allocator) {
    ...
    struct HashMap_usize_TypeUsage_Primitive other = (struct HashMap_usize_TypeUsage_Primitive) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_usize_TypeUsage_Primitive)),
        .length = self->length,
