...
struct HashMap_str_str HashMap_str_str__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_str self = (struct HashMap_str_str) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_str__resize(&self, initial_capacity);
...
struct str* HashMap_str_str__insert(struct HashMap_str_str* self, struct str key, struct str value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_str) { .key = key, .value = value };
