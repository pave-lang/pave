...
struct HashMap_str_StructField HashMap_str_StructField__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_StructField self = (struct HashMap_str_StructField) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_StructField__resize(&self, initial_capacity);
...
struct StructField* HashMap_str_StructField__insert(struct HashMap_str_StructField* self, struct str key, struct StructField value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_StructField) { .key = key, .value = value };
