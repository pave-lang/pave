...
#include <std/trait_Map_str_StructField.h>
#include <std/HashMap_str_StructField.h>
...
struct HashMap_str_StructField HashMap_str_StructField__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_StructField__with_capacity(allocator, 16);
}
...
struct HashMap_str_StructField HashMap_str_StructField__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_StructField self = (struct HashMap_str_StructField) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_StructField__resize(&self, capacity);
...
void HashMap_str_StructField__resize(struct HashMap_str_StructField* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_StructField*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_StructField));
    #line 56 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 57 "src/std/HashMap.pv"
    HashMap_str_StructField__fill_buckets(self);
}
...
struct StructField* HashMap_str_StructField__find(struct HashMap_str_StructField* self, struct str* key) {
    ...
    struct HashMapBucket_str_StructField* current_bucket_node = self->buckets[bucket_index];
...
struct StructField* HashMap_str_StructField__insert(struct HashMap_str_StructField* self, struct str key, struct StructField value) {
    ...
    struct StructField* existing_value = HashMap_str_StructField__find(self, &key);
    ...
    if (self->capacity == 0 || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_StructField__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_StructField* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_StructField) { .key = key, .value = value, .next = 0 };
    #line 99 "src/std/HashMap.pv"
    struct HashMapBucket_str_StructField* data = self->data + self->length;
...
bool HashMap_str_StructField__remove(struct HashMap_str_StructField* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (str__Hash__hash(&current_bucket_node->key) == hash) {
            ...
            struct HashMapBucket_str_StructField* last = self->data + self->length - 1;
...
void HashMap_str_StructField__fill_buckets(struct HashMap_str_StructField* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_StructField*));

    #line 143 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 144 "src/std/HashMap.pv"
        struct HashMapBucket_str_StructField* node = self->data + i;
...
struct HashMap_str_StructField HashMap_str_StructField__clone(struct HashMap_str_StructField* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_StructField other = (struct HashMap_str_StructField) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_StructField)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 169 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_StructField));
    #line 170 "src/std/HashMap.pv"
    HashMap_str_StructField__fill_buckets(&other);
...
struct HashMapIter_str_StructField HashMap_str_StructField__iter(struct HashMap_str_StructField* self) {
    ...
    return (struct HashMapIter_str_StructField) {
        .iter = self->data - 1,
...
void HashMap_str_StructField__clear(struct HashMap_str_StructField* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_StructField));
...
struct HashMapBucket_str_StructField* HashMap_str_StructField__Index__index(void* __self) {
    struct HashMap_str_StructField* self = __self; (void)self;
...
struct trait_Map_str_StructFieldVTable HASH_MAP_STR_STRUCT_FIELD__VTABLE__MAP = { .fn_find = &HashMap_str_StructField__Map_str_StructField__find, .fn_insert = &HashMap_str_StructField__Map_str_StructField__insert, .fn_remove = &HashMap_str_StructField__Map_str_StructField__remove };
