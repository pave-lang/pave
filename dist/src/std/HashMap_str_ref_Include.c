...
struct HashMap_str_ref_Include HashMap_str_ref_Include__new(struct Allocator allocator) {
    ...
    struct HashMap_str_ref_Include self = (struct HashMap_str_ref_Include) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_ref_Include__resize(&self, initial_capacity);
...
struct Include** HashMap_str_ref_Include__insert(struct HashMap_str_ref_Include* self, struct str key, struct Include* value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Include) { .key = key, .value = value };
