...
struct HashMap_str_usize HashMap_str_usize__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_usize__with_capacity(allocator, 16);
}
...
struct HashMap_str_usize HashMap_str_usize__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_usize self = (struct HashMap_str_usize) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_usize__resize(&self, capacity);
...
void HashMap_str_usize__resize(struct HashMap_str_usize* self, uintptr_t new_capacity) {
    ...
    HashMap_str_usize__fill_buckets(self);
}
...
uintptr_t* HashMap_str_usize__find(struct HashMap_str_usize* self, struct str* key) {
    ...
    struct HashMapBucket_str_usize* current_bucket_node = self->buckets[bucket_index];
...
uintptr_t* HashMap_str_usize__insert(struct HashMap_str_usize* self, struct str key, uintptr_t value) {
    ...
    uintptr_t* existing_value = HashMap_str_usize__find(self, &key);
    ...
    if (usize__Eq_usize__eq(self->capacity, 0) || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_usize__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_usize* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_usize) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_usize* data = self->data + self->length;
...
bool HashMap_str_usize__remove(struct HashMap_str_usize* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_usize* last = self->data + self->length - 1;
...
void HashMap_str_usize__fill_buckets(struct HashMap_str_usize* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_usize*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_usize* node = self->data + i;
...
struct HashMap_str_usize HashMap_str_usize__clone(struct HashMap_str_usize* self, struct trait_Allocator allocator) {
    ...
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_usize));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_usize__fill_buckets(&other);
...
struct HashMapIter_str_usize HashMap_str_usize__iter(struct HashMap_str_usize* self) {
    ...
    return (struct HashMapIter_str_usize) {
        .iter = self->data - 1,
...
void HashMap_str_usize__clear(struct HashMap_str_usize* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_usize));
...
struct HashMapBucket_str_usize* HashMap_str_usize__Index__index(void* __self) {
    struct HashMap_str_usize* self = __self; (void)self;
...
struct trait_Map_str_usizeVTable HASH_MAP_STR_USIZE__VTABLE__MAP = { .fn_find = &HashMap_str_usize__Map_str_usize__find, .fn_insert = &HashMap_str_usize__Map_str_usize__insert, .fn_remove = &HashMap_str_usize__Map_str_usize__remove };
