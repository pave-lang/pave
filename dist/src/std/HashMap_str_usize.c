...
struct HashMap_str_usize HashMap_str_usize__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_usize self = (struct HashMap_str_usize) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_usize__resize(&self, initial_capacity);
...
uintptr_t* HashMap_str_usize__insert(struct HashMap_str_usize* self, struct str key, uintptr_t value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_usize) { .key = key, .value = value };
