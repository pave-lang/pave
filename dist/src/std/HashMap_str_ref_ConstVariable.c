...
#include <std/HashMapBucket_str_ref_ConstVariable.h>
...
struct HashMap_str_ref_ConstVariable HashMap_str_ref_ConstVariable__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_ConstVariable self = (struct HashMap_str_ref_ConstVariable) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_ref_ConstVariable__resize(&self, initial_capacity);
...
void HashMap_str_ref_ConstVariable__resize(struct HashMap_str_ref_ConstVariable* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_ConstVariable*));
    #line 48 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_ConstVariable));
    #line 49 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 50 "src/std/HashMap.pv"
    HashMap_str_ref_ConstVariable__fill_buckets(self);
}
...
struct ConstVariable** HashMap_str_ref_ConstVariable__find(struct HashMap_str_ref_ConstVariable* self, struct str* key) {
    ...
    struct HashMapBucket_str_ref_ConstVariable* current_bucket_node = self->buckets[bucket_index];
...
struct ConstVariable** HashMap_str_ref_ConstVariable__insert(struct HashMap_str_ref_ConstVariable* self, struct str key, struct ConstVariable* value) {
    ...
    struct ConstVariable** existing_value = HashMap_str_ref_ConstVariable__find(self, &key);
    ...
    if (self->capacity == 0 || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_ref_ConstVariable__resize(self, self->capacity * 2);
    }
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_ConstVariable) { .key = key, .value = value };
    #line 90 "src/std/HashMap.pv"
    struct HashMapBucket_str_ref_ConstVariable* data = self->data + self->length;
...
bool HashMap_str_ref_ConstVariable__remove(struct HashMap_str_ref_ConstVariable* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (str__Hash__hash(&current_bucket_node->key) == hash) {
            ...
            struct HashMapBucket_str_ref_ConstVariable* last = self->data + self->length - 1;
...
void HashMap_str_ref_ConstVariable__fill_buckets(struct HashMap_str_ref_ConstVariable* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_ConstVariable*));

    #line 134 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 135 "src/std/HashMap.pv"
        struct HashMapBucket_str_ref_ConstVariable* node = self->data + i;
...
struct HashMap_str_ref_ConstVariable HashMap_str_ref_ConstVariable__clone(struct HashMap_str_ref_ConstVariable* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_ConstVariable other = (struct HashMap_str_ref_ConstVariable) {
        .allocator = allocator,
        .buckets = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_ConstVariable)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 157 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_ref_ConstVariable));
    #line 158 "src/std/HashMap.pv"
    HashMap_str_ref_ConstVariable__fill_buckets(&other);
...
struct HashMapIter_str_ref_ConstVariable HashMap_str_ref_ConstVariable__iter(struct HashMap_str_ref_ConstVariable* self) {
    ...
    return (struct HashMapIter_str_ref_ConstVariable) {
        .iter = self->data - 1,
...
void HashMap_str_ref_ConstVariable__clear(struct HashMap_str_ref_ConstVariable* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_ConstVariable));
