...
void Array_str__reserve(struct Array_str* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, capacity * sizeof(struct str));
...
struct Array_str Array_str__clone(struct Array_str* self, struct trait_Allocator allocator) {
    ...
    struct str* data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct str));
