...
struct Array_str Array_str__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_str) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_str Array_str__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_str self = (struct Array_str) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_str__reserve(&self, length);
...
void Array_str__reserve(struct Array_str* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct str));
...
uintptr_t Array_str__append(struct Array_str* self, struct str value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_str__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_str__prepend(struct Array_str* self, struct str value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct str));
...
bool Array_str__remove_back(struct Array_str* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct str));
...
void Array_str__clear(struct Array_str* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct str));
...
struct Array_str Array_str__clone(struct Array_str* self, struct trait_Allocator allocator) {
    ...
    struct str* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct str));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct str));

    #line 176 "src/std/Array.pv"
    return (struct Array_str) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
