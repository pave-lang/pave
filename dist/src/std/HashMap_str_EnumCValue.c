...
struct HashMap_str_EnumCValue HashMap_str_EnumCValue__new(struct Allocator allocator) {
    ...
    struct HashMap_str_EnumCValue self = (struct HashMap_str_EnumCValue) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_EnumCValue__resize(&self, initial_capacity);
...
struct EnumCValue* HashMap_str_EnumCValue__insert(struct HashMap_str_EnumCValue* self, struct str key, struct EnumCValue value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_EnumCValue) { .key = key, .value = value };
