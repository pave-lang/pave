...
void HashMap_str_Array_Diagnostic__resize(struct HashMap_str_Array_Diagnostic* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Array_Diagnostic*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Array_Diagnostic));
...
struct Array_Diagnostic* HashMap_str_Array_Diagnostic__insert(struct HashMap_str_Array_Diagnostic* self, struct str key, struct Array_Diagnostic value) {
    ...
    struct HashMapBucket_str_Array_Diagnostic* bucket_node = *current_bucket_node;
...
struct HashMap_str_Array_Diagnostic HashMap_str_Array_Diagnostic__clone(struct HashMap_str_Array_Diagnostic* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Array_Diagnostic other = (struct HashMap_str_Array_Diagnostic) {
        ...
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Array_Diagnostic)),
        .length = self->length,
...
struct HashMapBucket_str_Array_Diagnostic* HashMap_str_Array_Diagnostic__Index__index(void* __self) {
    struct HashMap_str_Array_Diagnostic* self = __self; (void)self;
...
struct trait_Map_str_Array_DiagnosticVTable HASH_MAP_STR_ARRAY_DIAGNOSTIC__VTABLE__MAP = { .fn_find = &HashMap_str_Array_Diagnostic__Map_str_Array_Diagnostic__find, .fn_insert = &HashMap_str_Array_Diagnostic__Map_str_Array_Diagnostic__insert, .fn_remove = &HashMap_str_Array_Diagnostic__Map_str_Array_Diagnostic__remove };
