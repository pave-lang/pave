...
struct HashMap_str_Array_Diagnostic HashMap_str_Array_Diagnostic__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Array_Diagnostic self = (struct HashMap_str_Array_Diagnostic) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_Array_Diagnostic__resize(&self, initial_capacity);
...
struct Array_Diagnostic* HashMap_str_Array_Diagnostic__insert(struct HashMap_str_Array_Diagnostic* self, struct str key, struct Array_Diagnostic value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_Array_Diagnostic) { .key = key, .value = value };
