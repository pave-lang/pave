...
struct Array_Generic Array_Generic__new(struct Allocator allocator) {
    #line 78 "src/std/Array.pv"
    return (struct Array_Generic) { .allocator = allocator };
}

#line 81 "src/std/Array.pv"
struct Array_Generic Array_Generic__new_with_length(struct Allocator allocator, uintptr_t length) {
    #line 82 "src/std/Array.pv"
    struct Array_Generic self = (struct Array_Generic) { .allocator = allocator };
    #line 83 "src/std/Array.pv"
    Array_Generic__reserve(&self, length);
...
struct Array_Generic Array_Generic__new_with_capacity(struct Allocator allocator, uintptr_t length) {
...
void Array_Generic__reserve(struct Array_Generic* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, capacity * sizeof(struct Generic));
...
uintptr_t Array_Generic__append(struct Array_Generic* self, struct Generic value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_Generic__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_Generic__prepend(struct Array_Generic* self, struct Generic value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct Generic));
...
bool Array_Generic__remove_back(struct Array_Generic* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct Generic));
...
void Array_Generic__clear(struct Array_Generic* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct Generic));
...
struct Array_Generic Array_Generic__clone(struct Array_Generic* self, struct Allocator allocator) {
    #line 173 "src/std/Array.pv"
    struct Generic* data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct Generic));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Generic));

    #line 176 "src/std/Array.pv"
    return (struct Array_Generic) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
