...
struct Array_Generic Array_Generic__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_Generic) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_Generic Array_Generic__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_Generic self = (struct Array_Generic) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
...
void Array_Generic__reserve(struct Array_Generic* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct Generic));
...
struct Array_Generic Array_Generic__clone(struct Array_Generic* self, struct trait_Allocator allocator) {
    ...
    struct Generic* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct Generic));
...
struct Iter_ref_Generic Array_Generic__iter(struct Array_Generic* self) {
    ...
    return Iter_ref_Generic__new(self->data, self->data + self->length);
}
