...
struct Array_ref_Struct Array_ref_Struct__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_ref_Struct) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_ref_Struct Array_ref_Struct__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_ref_Struct self = (struct Array_ref_Struct) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_ref_Struct__reserve(&self, length);
...
void Array_ref_Struct__reserve(struct Array_ref_Struct* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct Struct*));
...
uintptr_t Array_ref_Struct__append(struct Array_ref_Struct* self, struct Struct* value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_ref_Struct__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_ref_Struct__prepend(struct Array_ref_Struct* self, struct Struct* value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct Struct*));
...
bool Array_ref_Struct__remove_back(struct Array_ref_Struct* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct Struct*));
...
void Array_ref_Struct__clear(struct Array_ref_Struct* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct Struct*));
...
struct Array_ref_Struct Array_ref_Struct__clone(struct Array_ref_Struct* self, struct trait_Allocator allocator) {
    ...
    struct Struct** data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct Struct*));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Struct*));

    #line 176 "src/std/Array.pv"
    return (struct Array_ref_Struct) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct slice_ref_Struct Array_ref_Struct__as_slice(struct Array_ref_Struct* self) {
    ...
    return (struct slice_ref_Struct) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct Struct** Array_ref_Struct__Index__index(void* __self) {
    struct Array_ref_Struct* self = __self; (void)self;
