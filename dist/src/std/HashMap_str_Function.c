...
struct HashMap_str_Function HashMap_str_Function__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Function self = (struct HashMap_str_Function) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_Function__resize(&self, initial_capacity);
...
struct Function* HashMap_str_Function__insert(struct HashMap_str_Function* self, struct str key, struct Function value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_Function) { .key = key, .value = value };
