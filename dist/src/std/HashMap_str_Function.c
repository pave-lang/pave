...
void HashMap_str_Function__resize(struct HashMap_str_Function* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Function*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Function));
...
struct HashMap_str_Function HashMap_str_Function__clone(struct HashMap_str_Function* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Function other = (struct HashMap_str_Function) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Function)),
        .length = self->length,
