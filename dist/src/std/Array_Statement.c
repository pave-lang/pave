...
struct Array_Statement Array_Statement__new(struct Allocator allocator) {
    #line 78 "src/std/Array.pv"
    return (struct Array_Statement) { .allocator = allocator };
}

#line 81 "src/std/Array.pv"
struct Array_Statement Array_Statement__new_with_length(struct Allocator allocator, uintptr_t length) {
    #line 82 "src/std/Array.pv"
    struct Array_Statement self = (struct Array_Statement) { .allocator = allocator };
    #line 83 "src/std/Array.pv"
    Array_Statement__reserve(&self, length);
...
struct Array_Statement Array_Statement__new_with_capacity(struct Allocator allocator, uintptr_t length) {
...
void Array_Statement__reserve(struct Array_Statement* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, capacity * sizeof(struct Statement));
...
uintptr_t Array_Statement__append(struct Array_Statement* self, struct Statement value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_Statement__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_Statement__prepend(struct Array_Statement* self, struct Statement value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct Statement));
...
bool Array_Statement__remove_back(struct Array_Statement* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct Statement));
...
void Array_Statement__clear(struct Array_Statement* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct Statement));
...
struct Array_Statement Array_Statement__clone(struct Array_Statement* self, struct Allocator allocator) {
    #line 173 "src/std/Array.pv"
    struct Statement* data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct Statement));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Statement));

    #line 176 "src/std/Array.pv"
    return (struct Array_Statement) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
