...
struct Array_ref_Expression Array_ref_Expression__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_ref_Expression) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_ref_Expression Array_ref_Expression__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_ref_Expression self = (struct Array_ref_Expression) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_ref_Expression__reserve(&self, length);
...
void Array_ref_Expression__reserve(struct Array_ref_Expression* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct Expression*));
...
uintptr_t Array_ref_Expression__append(struct Array_ref_Expression* self, struct Expression* value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_ref_Expression__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_ref_Expression__prepend(struct Array_ref_Expression* self, struct Expression* value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct Expression*));
...
bool Array_ref_Expression__remove_back(struct Array_ref_Expression* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct Expression*));
...
void Array_ref_Expression__clear(struct Array_ref_Expression* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct Expression*));
...
struct Array_ref_Expression Array_ref_Expression__clone(struct Array_ref_Expression* self, struct trait_Allocator allocator) {
    ...
    struct Expression** data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct Expression*));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Expression*));

    #line 176 "src/std/Array.pv"
    return (struct Array_ref_Expression) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct Expression** Array_ref_Expression__Index__index(void* __self) {
    struct Array_ref_Expression* self = __self; (void)self;
