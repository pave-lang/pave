...
struct HashMap_str_Trait HashMap_str_Trait__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_Trait__with_capacity(allocator, 16);
}

#line 39 "src/std/HashMap.pv"
struct HashMap_str_Trait HashMap_str_Trait__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    #line 40 "src/std/HashMap.pv"
    struct HashMap_str_Trait self = (struct HashMap_str_Trait) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_Trait__resize(&self, capacity);
...
void HashMap_str_Trait__resize(struct HashMap_str_Trait* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Trait*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Trait));
    #line 56 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 57 "src/std/HashMap.pv"
    HashMap_str_Trait__fill_buckets(self);
}
...
struct Trait* HashMap_str_Trait__find(struct HashMap_str_Trait* self, struct str* key) {
    ...
    struct HashMapBucket_str_Trait* current_bucket_node = self->buckets[bucket_index];
...
struct Trait* HashMap_str_Trait__insert(struct HashMap_str_Trait* self, struct str key, struct Trait value) {
    ...
    struct Trait* existing_value = HashMap_str_Trait__find(self, &key);
    ...
    if (self->capacity == 0 || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_Trait__resize(self, self->capacity * 2);
    }
    ...
    self->data[self->length] = (struct HashMapBucket_str_Trait) { .key = key, .value = value, .next = 0 };
    #line 97 "src/std/HashMap.pv"
    struct HashMapBucket_str_Trait* data = self->data + self->length;
...
bool HashMap_str_Trait__remove(struct HashMap_str_Trait* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (str__Hash__hash(&current_bucket_node->key) == hash) {
            ...
            struct HashMapBucket_str_Trait* last = self->data + self->length - 1;
...
void HashMap_str_Trait__fill_buckets(struct HashMap_str_Trait* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_Trait*));

    #line 141 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 142 "src/std/HashMap.pv"
        struct HashMapBucket_str_Trait* node = self->data + i;
...
struct HashMap_str_Trait HashMap_str_Trait__clone(struct HashMap_str_Trait* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Trait other = (struct HashMap_str_Trait) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Trait)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 164 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_Trait));
    #line 165 "src/std/HashMap.pv"
    HashMap_str_Trait__fill_buckets(&other);
...
struct HashMapIter_str_Trait HashMap_str_Trait__iter(struct HashMap_str_Trait* self) {
    ...
    return (struct HashMapIter_str_Trait) {
        .iter = self->data - 1,
...
void HashMap_str_Trait__clear(struct HashMap_str_Trait* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_Trait));
...
struct HashMapBucket_str_Trait* HashMap_str_Trait__Index__index(void* __self) {
    struct HashMap_str_Trait* self = __self; (void)self;
