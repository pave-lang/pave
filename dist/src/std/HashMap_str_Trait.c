...
void HashMap_str_Trait__resize(struct HashMap_str_Trait* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Trait*));
    #line 55 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Trait));
...
struct HashMap_str_Trait HashMap_str_Trait__clone(struct HashMap_str_Trait* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Trait other = (struct HashMap_str_Trait) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Trait)),
        .length = self->length,
