...
struct HashMap_str_Trait HashMap_str_Trait__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Trait self = (struct HashMap_str_Trait) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_Trait__resize(&self, initial_capacity);
...
struct Trait* HashMap_str_Trait__insert(struct HashMap_str_Trait* self, struct str key, struct Trait value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_Trait) { .key = key, .value = value };
