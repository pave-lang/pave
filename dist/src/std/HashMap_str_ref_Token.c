...
struct HashMap_str_ref_Token HashMap_str_ref_Token__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_ref_Token__with_capacity(allocator, 16);
}
...
struct HashMap_str_ref_Token HashMap_str_ref_Token__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_ref_Token self = (struct HashMap_str_ref_Token) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_ref_Token__resize(&self, capacity);
...
void HashMap_str_ref_Token__resize(struct HashMap_str_ref_Token* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_Token*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_Token));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_ref_Token__fill_buckets(self);
}
...
struct Token** HashMap_str_ref_Token__find(struct HashMap_str_ref_Token* self, struct str* key) {
    ...
    struct HashMapBucket_str_ref_Token* current_bucket_node = self->buckets[bucket_index];
...
struct Token** HashMap_str_ref_Token__insert(struct HashMap_str_ref_Token* self, struct str key, struct Token* value) {
    ...
    struct Token** existing_value = HashMap_str_ref_Token__find(self, &key);
    ...
    if (usize__Eq_usize__eq(self->capacity, 0) || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_ref_Token__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_ref_Token* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Token) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_ref_Token* data = self->data + self->length;
...
bool HashMap_str_ref_Token__remove(struct HashMap_str_ref_Token* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_ref_Token* last = self->data + self->length - 1;
...
struct HashMap_str_ref_Token HashMap_str_ref_Token__clone(struct HashMap_str_ref_Token* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Token other = (struct HashMap_str_ref_Token) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_Token)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_ref_Token));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_ref_Token__fill_buckets(&other);
...
struct HashMapIter_str_ref_Token HashMap_str_ref_Token__iter(struct HashMap_str_ref_Token* self) {
    ...
    return (struct HashMapIter_str_ref_Token) {
        .iter = self->data - 1,
...
void HashMap_str_ref_Token__clear(struct HashMap_str_ref_Token* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Token));
...
struct HashMapBucket_str_ref_Token* HashMap_str_ref_Token__Index__index(void* __self) {
    struct HashMap_str_ref_Token* self = __self; (void)self;
...
struct trait_Map_str_ref_TokenVTable HASH_MAP_STR_REF_TOKEN__VTABLE__MAP = { .fn_find = &HashMap_str_ref_Token__Map_str_ref_Token__find, .fn_insert = &HashMap_str_ref_Token__Map_str_ref_Token__insert, .fn_remove = &HashMap_str_ref_Token__Map_str_ref_Token__remove };
