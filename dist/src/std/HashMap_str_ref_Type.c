...
#include <std/HashMapBucket_str_ref_Type.h>
#include <usize.h>
#include <std/Hash.h>
...
struct HashMap_str_ref_Type HashMap_str_ref_Type__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_ref_Type__with_capacity(allocator, 16);
}
...
struct HashMap_str_ref_Type HashMap_str_ref_Type__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    ...
    struct HashMap_str_ref_Type self = (struct HashMap_str_ref_Type) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_ref_Type__resize(&self, capacity);
...
void HashMap_str_ref_Type__resize(struct HashMap_str_ref_Type* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_Type*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_Type));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_ref_Type__fill_buckets(self);
}
...
struct Type** HashMap_str_ref_Type__find(struct HashMap_str_ref_Type* self, struct str* key) {
    ...
    struct HashMapBucket_str_ref_Type* current_bucket_node = self->buckets[bucket_index];
...
struct Type** HashMap_str_ref_Type__insert(struct HashMap_str_ref_Type* self, struct str key, struct Type* value) {
    ...
    struct Type** existing_value = HashMap_str_ref_Type__find(self, &key);
    ...
    struct HashMapBucket_str_ref_Type* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Type) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_ref_Type* data = self->data + self->length;
...
bool HashMap_str_ref_Type__remove(struct HashMap_str_ref_Type* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_ref_Type* last = self->data + self->length - 1;
...
void HashMap_str_ref_Type__fill_buckets(struct HashMap_str_ref_Type* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Type*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_ref_Type* node = self->data + i;
...
struct HashMap_str_ref_Type HashMap_str_ref_Type__clone(struct HashMap_str_ref_Type* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Type other = (struct HashMap_str_ref_Type) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_Type)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_ref_Type));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_ref_Type__fill_buckets(&other);
...
struct HashMapIter_str_ref_Type HashMap_str_ref_Type__iter(struct HashMap_str_ref_Type* self) {
    ...
    return (struct HashMapIter_str_ref_Type) {
        .iter = self->data - 1,
...
void HashMap_str_ref_Type__clear(struct HashMap_str_ref_Type* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Type));
...
struct HashMapBucket_str_ref_Type* HashMap_str_ref_Type__Index__index(void* __self) {
    struct HashMap_str_ref_Type* self = __self; (void)self;
...
struct trait_Map_str_ref_TypeVTable HASH_MAP_STR_REF_TYPE__VTABLE__MAP = { .fn_find = &HashMap_str_ref_Type__Map_str_ref_Type__find, .fn_insert = &HashMap_str_ref_Type__Map_str_ref_Type__insert, .fn_remove = &HashMap_str_ref_Type__Map_str_ref_Type__remove };
