...
struct HashMap_str_ref_Type HashMap_str_ref_Type__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Type self = (struct HashMap_str_ref_Type) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_ref_Type__resize(&self, initial_capacity);
...
struct Type** HashMap_str_ref_Type__insert(struct HashMap_str_ref_Type* self, struct str key, struct Type* value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Type) { .key = key, .value = value };
