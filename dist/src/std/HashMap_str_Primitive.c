...
struct HashMap_str_Primitive HashMap_str_Primitive__new(struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Primitive self = (struct HashMap_str_Primitive) { .allocator = allocator };

    #line 41 "src/std/HashMap.pv"
    HashMap_str_Primitive__resize(&self, initial_capacity);
...
void HashMap_str_Primitive__resize(struct HashMap_str_Primitive* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_Primitive*));
    #line 48 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_Primitive));
...
struct Primitive* HashMap_str_Primitive__insert(struct HashMap_str_Primitive* self, struct str key, struct Primitive value) {
    ...
    self->data[self->length] = (struct HashMapBucket_str_Primitive) { .key = key, .value = value };
...
struct HashMap_str_Primitive HashMap_str_Primitive__clone(struct HashMap_str_Primitive* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_Primitive other = (struct HashMap_str_Primitive) {
        ...
        .data = allocator.vtable->alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_Primitive)),
        .length = self->length,
