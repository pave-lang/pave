...
struct HashMap_str_ref_Module HashMap_str_ref_Module__new(struct trait_Allocator allocator) {
    ...
    return HashMap_str_ref_Module__with_capacity(allocator, 16);
}

#line 40 "src/std/HashMap.pv"
struct HashMap_str_ref_Module HashMap_str_ref_Module__with_capacity(struct trait_Allocator allocator, uintptr_t capacity) {
    #line 41 "src/std/HashMap.pv"
    struct HashMap_str_ref_Module self = (struct HashMap_str_ref_Module) {
        .allocator = allocator,
        .buckets = 0,
        .data = 0,
        .capacity = 0,
    ...
    HashMap_str_ref_Module__resize(&self, capacity);
...
void HashMap_str_ref_Module__resize(struct HashMap_str_ref_Module* self, uintptr_t new_capacity) {
    ...
    self->buckets = self->allocator.vtable->fn_realloc(self->allocator.instance, self->buckets, new_capacity * sizeof(struct HashMapBucket_str_ref_Module*));
    #line 56 "src/std/HashMap.pv"
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, new_capacity * sizeof(struct HashMapBucket_str_ref_Module));
    #line 57 "src/std/HashMap.pv"
    self->capacity = new_capacity;
    #line 58 "src/std/HashMap.pv"
    HashMap_str_ref_Module__fill_buckets(self);
}
...
struct Module** HashMap_str_ref_Module__find(struct HashMap_str_ref_Module* self, struct str* key) {
    ...
    struct HashMapBucket_str_ref_Module* current_bucket_node = self->buckets[bucket_index];
...
struct Module** HashMap_str_ref_Module__insert(struct HashMap_str_ref_Module* self, struct str key, struct Module* value) {
    ...
    struct Module** existing_value = HashMap_str_ref_Module__find(self, &key);
    ...
    if (usize__Eq_usize__eq(self->capacity, 0) || (self->length * 100 / self->capacity) > 75) {
        ...
        HashMap_str_ref_Module__resize(self, self->capacity * 2);
    }
    ...
    struct HashMapBucket_str_ref_Module* bucket_node = *current_bucket_node;
    ...
    self->data[self->length] = (struct HashMapBucket_str_ref_Module) { .key = key, .value = value, .next = 0 };
    #line 100 "src/std/HashMap.pv"
    struct HashMapBucket_str_ref_Module* data = self->data + self->length;
...
bool HashMap_str_ref_Module__remove(struct HashMap_str_ref_Module* self, struct str* key) {
    ...
    while (current_bucket_node != 0) {
        ...
        if (u64__Eq_u64__eq(str__Hash__hash(&current_bucket_node->key), hash) && str__Eq_str__eq(current_bucket_node->key, *key)) {
            ...
            struct HashMapBucket_str_ref_Module* last = self->data + self->length - 1;
...
void HashMap_str_ref_Module__fill_buckets(struct HashMap_str_ref_Module* self) {
    ...
    memset(self->buckets, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Module*));

    #line 144 "src/std/HashMap.pv"
    for (uintptr_t i = 0; i < self->length; i++) {
        #line 145 "src/std/HashMap.pv"
        struct HashMapBucket_str_ref_Module* node = self->data + i;
...
struct HashMap_str_ref_Module HashMap_str_ref_Module__clone(struct HashMap_str_ref_Module* self, struct trait_Allocator allocator) {
    ...
    struct HashMap_str_ref_Module other = (struct HashMap_str_ref_Module) {
        .allocator = allocator,
        .buckets = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(self->data)),
        .data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct HashMapBucket_str_ref_Module)),
        .length = self->length,
        .capacity = self->capacity,
    };
    #line 170 "src/std/HashMap.pv"
    memcpy(other.data, self->data, self->capacity * sizeof(struct HashMapBucket_str_ref_Module));
    #line 171 "src/std/HashMap.pv"
    HashMap_str_ref_Module__fill_buckets(&other);
...
struct HashMapIter_str_ref_Module HashMap_str_ref_Module__iter(struct HashMap_str_ref_Module* self) {
    ...
    return (struct HashMapIter_str_ref_Module) {
        .iter = self->data - 1,
...
void HashMap_str_ref_Module__clear(struct HashMap_str_ref_Module* self) {
    ...
    memset(self->data, 0, self->capacity * sizeof(struct HashMapBucket_str_ref_Module));
...
struct HashMapBucket_str_ref_Module* HashMap_str_ref_Module__Index__index(void* __self) {
    struct HashMap_str_ref_Module* self = __self; (void)self;
...
struct trait_Map_str_ref_ModuleVTable HASH_MAP_STR_REF_MODULE__VTABLE__MAP = { .fn_find = &HashMap_str_ref_Module__Map_str_ref_Module__find, .fn_insert = &HashMap_str_ref_Module__Map_str_ref_Module__insert, .fn_remove = &HashMap_str_ref_Module__Map_str_ref_Module__remove };
