...
#include <stdint.h>
#include <tuple_usize_ref_ref_Tuple.h>
...
bool IterEnumerate_ref_ref_Tuple__next(struct IterEnumerate_ref_ref_Tuple* self) {
    ...
    if (!ArrayIter_ref_ref_Tuple__next(&self->iter)) {
        #line 59 "src/std/Array.pv"
        return false;
    }
...
struct tuple_usize_ref_ref_Tuple IterEnumerate_ref_ref_Tuple__value(struct IterEnumerate_ref_ref_Tuple* self) {
    ...
    return (struct tuple_usize_ref_ref_Tuple) { ._0 = self->index - 1, ._1 = ArrayIter_ref_ref_Tuple__value(&self->iter) };
}
