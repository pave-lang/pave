...
struct Array_Type Array_Type__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_Type) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}

#line 81 "src/std/Array.pv"
struct Array_Type Array_Type__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    #line 82 "src/std/Array.pv"
    struct Array_Type self = (struct Array_Type) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_Type__reserve(&self, length);
...
struct Array_Type Array_Type__new_with_capacity(struct trait_Allocator allocator, uintptr_t length) {
...
void Array_Type__reserve(struct Array_Type* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct Type));
...
uintptr_t Array_Type__append(struct Array_Type* self, struct Type value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_Type__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_Type__prepend(struct Array_Type* self, struct Type value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct Type));
...
bool Array_Type__remove_back(struct Array_Type* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct Type));
...
void Array_Type__clear(struct Array_Type* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct Type));
...
struct Array_Type Array_Type__clone(struct Array_Type* self, struct trait_Allocator allocator) {
    #line 173 "src/std/Array.pv"
    struct Type* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct Type));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct Type));
...
struct Iter_ref_Type Array_Type__iter(struct Array_Type* self) {
