...
struct Array_TestInfo Array_TestInfo__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_TestInfo) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_TestInfo Array_TestInfo__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_TestInfo self = (struct Array_TestInfo) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_TestInfo__reserve(&self, length);
...
void Array_TestInfo__reserve(struct Array_TestInfo* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct TestInfo));
...
uintptr_t Array_TestInfo__append(struct Array_TestInfo* self, struct TestInfo value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_TestInfo__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_TestInfo__prepend(struct Array_TestInfo* self, struct TestInfo value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct TestInfo));
...
bool Array_TestInfo__remove_back(struct Array_TestInfo* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct TestInfo));
...
void Array_TestInfo__clear(struct Array_TestInfo* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct TestInfo));
...
struct Array_TestInfo Array_TestInfo__clone(struct Array_TestInfo* self, struct trait_Allocator allocator) {
    ...
    struct TestInfo* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct TestInfo));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct TestInfo));

    #line 176 "src/std/Array.pv"
    return (struct Array_TestInfo) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct slice_TestInfo Array_TestInfo__as_slice(struct Array_TestInfo* self) {
    ...
    return (struct slice_TestInfo) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct TestInfo* Array_TestInfo__Index__index(void* __self) {
    struct Array_TestInfo* self = __self; (void)self;
