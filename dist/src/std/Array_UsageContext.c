...
struct Array_UsageContext Array_UsageContext__new(struct trait_Allocator allocator) {
    ...
    return (struct Array_UsageContext) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
}
...
struct Array_UsageContext Array_UsageContext__new_with_length(struct trait_Allocator allocator, uintptr_t length) {
    ...
    struct Array_UsageContext self = (struct Array_UsageContext) { .allocator = allocator, .data = 0, .length = 0, .capacity = 0 };
    #line 83 "src/std/Array.pv"
    Array_UsageContext__reserve(&self, length);
...
void Array_UsageContext__reserve(struct Array_UsageContext* self, uintptr_t capacity) {
    ...
    self->data = self->allocator.vtable->fn_realloc(self->allocator.instance, self->data, capacity * sizeof(struct UsageContext));
...
uintptr_t Array_UsageContext__append(struct Array_UsageContext* self, struct UsageContext value) {
    ...
    if (self->capacity <= self->length + 1) {
        ...
        Array_UsageContext__reserve(self, self->capacity + additional_capacity);
    }
...
uintptr_t Array_UsageContext__prepend(struct Array_UsageContext* self, struct UsageContext value) {
    ...
    memmove(self->data + 1, self->data, self->length * sizeof(struct UsageContext));
...
bool Array_UsageContext__remove_back(struct Array_UsageContext* self) {
    ...
    memset(self->data + self->length, 0, sizeof(struct UsageContext));
...
void Array_UsageContext__clear(struct Array_UsageContext* self) {
    ...
    memset(self->data, 0, self->length * sizeof(struct UsageContext));
...
struct Array_UsageContext Array_UsageContext__clone(struct Array_UsageContext* self, struct trait_Allocator allocator) {
    ...
    struct UsageContext* data = allocator.vtable->fn_alloc(allocator.instance, self->capacity * sizeof(struct UsageContext));
    #line 174 "src/std/Array.pv"
    memcpy(data, self->data, self->length * sizeof(struct UsageContext));

    #line 176 "src/std/Array.pv"
    return (struct Array_UsageContext) {
        .allocator = allocator,
        .data = data,
        .length = self->length,
...
struct slice_UsageContext Array_UsageContext__as_slice(struct Array_UsageContext* self) {
    ...
    return (struct slice_UsageContext) { .data = self->data, .length = self->length };
}

#line 195 "src/std/Array.pv"
struct UsageContext* Array_UsageContext__Index__index(void* __self) {
    struct Array_UsageContext* self = __self; (void)self;
